...
def get_eligible_people(day, shift, people, shift_counts, night_counts, current_assignments):
    ...
    debug_log(f"\nGetting available people for {day} {shift}")
    debug_log("================================================")
    ...
    for person in people:
        ...
        if shift == "Morning":
            if previous_day and is_shift_assigned(person, previous_day, "Night", current_assignments):
                ...
                debug_log(f"{person['name']} not eligible: Already assigned {previous_day} night")
                continue
            ...
            if not(person['double_shift']) and person in current_assignments[day][next_shift]:
                ...
                debug_log(f"{person['name']} not eligible: No double shifts allowed. Already assigned for {day} {next_shift}")
                continue
            ...
            if not(person["double_shift"]) and (person in current_assignments[day][next_shift] or person in current_assignments[day][previous_shift]):
                # Check if person allows consecutive shifts and whether it's assigned
                debug_log(f"{person['name']} not eligible: No double shifts allowed. Already assigned for {day} {next_shift} or {previous_shift}")
                continue
            ...
            if is_shift_assigned(person, day, "Night", current_assignments):
                ...
                debug_log(f"{person['name']} not eligible: Already assigned {day} night")
                continue
            if not(person["double_shift"]) and person in current_assignments[day][previous_shift]:
                # Check if person allows consecutive shifts and whether it's assigned
                debug_log(f"{person['name']} not eligible: No double shifts allowed. Already assigned for {day} {previous_shift}")
                continue
            ...
            if night_counts[person['name']] >= person['max_nights']:
                ...
                debug_log(f"\n{person['name']} not eligible: Max night limitation breached ({person['max_nights']})")
                continue
            ...
            if next_day and is_shift_assigned(person, next_day, "Morning", current_assignments):
                ...
                debug_log(f"{person['name']} not eligible: Already assigned {next_day} morning")
                continue
            ...
            if next_day and is_shift_assigned(person, next_day, "Noon", current_assignments):
                ...
                debug_log(f"{person['name']} not eligible: Already assigned {next_day} noon")
                continue
            if is_shift_assigned(person, next_day, "Evening", current_assignments):
                # Check that no shift assigned today evening
                debug_log(f"{person['name']} not eligible: Already assigned {day} Evening")
                continue
    
        # Add eligible person
        debug_log(f"{person['name']} is eligible for {day} {shift}")
